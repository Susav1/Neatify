...
import "../styles/userManagement.css"; // Reusing the same styles
  ...
  const fetchCleaners = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/admin/getAllCleaners"
      );
      if (!response.ok) {
    ...
    <div className="user-management-container">
      <div className="header-section">
        ...
        <div className="controls">
          ...
          <button onClick={fetchCleaners} className="refresh-button">
      ...
      <div className="table-container">
        {isLoading ? (
          ...
          <div className="error-message">
            ...
            <p>{error}</p>
            <button onClick={fetchCleaners} className="retry-button">
              Retry
            </button>
          </div>
        ) : (
            ...
            <tbody>
              {filteredCleaners.length > 0 ? (
                filteredCleaners.map((cleaner) => (
                  <tr key={cleaner.id}>
                    <td className="user-id">#{cleaner.id}</td>
                    <td className="user-name">
                      <div className="avatar-placeholder">{cleaner.name}</div>
                    </td>
