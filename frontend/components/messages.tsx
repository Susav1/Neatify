...
const Messages: React.FC<MessagesProps> = ({ setCurrentPage }) => {
  ...
  }, [authState.authenticated, setCurrentPage]);
      ...
      <SafeAreaView style={tw`flex-1 bg-white`}>
        <View style={tw`flex-row items-center p-4 border-b border-gray-200`}>
          ...
          <Text style={tw`ml-3 text-lg font-semibold text-gray-800`}>
            {conversations.find((c) => c.id === selectedConversation)?.cleaner?.name || 'Chat'}
          </Text>
        </View>
        <FlatList
          data={messages}
